...
function mapInit() {
	try {
        ...
        var options={
        		titleNext:getNls("B_NAVIGATION_NEXT"),
				titleBack:getNls("B_NAVIGATION_BACK")
        };
        ...
        mp.formatOutput= function(lonLat) {
			...
			getNls(projeccio) + this.suffix + '<br>' + this.prefix  + lonLat.lon.toFixed(0) + this.separator + lonLat.lat.toFixed(0)  +
			"</font>";
			return newHtml;
		};
...
function setMapConfig(olConfig,map) {
    ...
    if (olConfig.scales!=null && olConfig.scales.length>0){
			var isOn=sitmunProperties["mapa.lista.escalas"] && sitmunProperties["mapa.lista.escalas"]=="territorio";
			...
			for(var nesc=olConfig.scales.length-1;nesc>=0;nesc--){
				var res=OpenLayers.Util.getResolutionFromScale(olConfig.scales[nesc],map.units);
				//console.log(idealResolution,"    ",olConfig.scales[nesc],"   ", res, "     ",(res<=idealResolution));
				 resArray.push(res);
				 if (isOn && res>idealResolution) nesc=-1;
			}
...
function callLegend(json){
	try{
		...
		if (typeof(legendResult.errorMessage) != "undefined" && legendResult.errorMessage !="") { 
			...
			var x=window.open(legendResult.LegendUrls[0].url);
...
function centrarEquipament(x, y){
	try{
		console.log("Inici del centrarRuta");
		console.log("x", x);
		console.log("y", y);
		if (x != "" && y != "" )
			map.zoomToExtent(new OpenLayers.Bounds(x-100,y-100,x+100, y+100));
