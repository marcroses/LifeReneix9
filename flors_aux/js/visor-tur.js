...
function legendInit(treeJSON) {
	...
	try {
		...
		globalFirstOpen=true;
		// buscamos las carpetas para crear un titlePane, y para cada hijo (cartografia) un checkbox.
		store.fetch( { query: { cartoid: '0' },  queryOptions: {deep: true},
			onComplete: function(items,query) {
				try {
					for (var i=0;i<items.length ; i++){
						var item=items[i];
						var id=store.getIdentity(item);
						//console.log("legend init item: ", id, "  ",item);
						var content="";
						aNew[i] = new Array();
						nodeWidget={"label": item.arnNombre, "item":item};
						aNew[i].push(nodeWidget);
						// recorremos las cartografias para crear los checkboxs.
						for ( var j=0; j < item.children.length; j++) {
							nodeWidget2={"label": item.children[j].arnNombre, "item":item.children[j]};
							aNew[i].push(nodeWidget2);
							
							var node = item.children[j];
							var img = getNls(node.arnNombre+"_IMG");
							if (img==node.arnNombre+"_IMG") img="img/arbre/capa_blank.png";
							content+='<div class="treeCartoItem">'
											+'<input name="mycheck'+j+'" class="treeCartoItemCheck" dojoType="dijit.form.CheckBox" value="agreed" '+
									        ((node.arnActivo[0])?'checked':'')+' onChange="treeNodeChecked2('+node.arnCodigo+');" />'+
									        '<label for="mycheck'+j+'" class="treeCartoItemLabel">'+
									        	'<img class="treeCartoItemImg" src="'+img+'"/><span oncontextmenu="findWidget(' + i +', '+(j +1)+');">' +getNls(node.arnNombre)+'</span>'+
									        '</label>'+
									'</div>';
						}
						var img = getNls(item.arnNombre+"_IMG");
						if (img==item.arnNombre+"_IMG") img="img/arbre/grup_blank.png";
						var tp = new dijit.TitlePane({
							title: '<img class="dijitTitlePaneImg" src="'+img+'"/>'
									+'<span oncontextmenu="findWidget(' + i +',0);">'+getNls(item.arnNombre)+'</span>',
							content: content,
							open: globalFirstOpen
						});
						globalFirstOpen=false;
						dojo.byId("treeOne").appendChild(tp.domNode);
					}
				} catch (e) {
					console.error(e);
				}
		...
		store.fetch({
		    query: {'arnActivo': true },
		    onComplete: this.fetchVisibleNodesCompleteImprimir,
		    onError: this.fetchVisibleNodesErrorImprimir,// TODO
		...
		dojo.connect( tree,"onNodeUnchecked",tree, treeNodeChecked);
...
function findWidget(i, j){
	//item = aNew[i][j];
	//console.log("itemSeleccionat=", item);
	//if (item != null)
	treeSelectedNode = aNew[i][j];
	//else treeSelectedNode = null; //por si no devuelve nada de la array
}
...
function treeNodeClicked(storeItem,nodeWidget) { 
	console.log(">> treeNodeClicked",nodeWidget);
	...
	treeContextMenu.getChildren().forEach(function(i) {
		if (i.label==getNls("LEYENDA")) {
			...
			var enabled=carto && typeof(carto.carLeyendurl)=="string";
	...
	console.log("treeSelectedNode.label", treeSelectedNode.label);
 ...
 * @return
 */
function treeNodeClicked2(storeItem,nodeWidget) { 
	console.log(">> treeNodeClicked2",nodeWidget);
	
	var isCarpeta=nodeWidget.item.cartoid[0]==0;
	if (isCarpeta) {
		console.log("treeNodeClicked2 - es una carpeta");
		treeContextMenu.getChildren().forEach(function(i) {// deshab. todos
			i.attr('disabled', true);
		});
	} 
	...
	dojo.byId("dialogTransparenciaLayer").innerHTML=getNls(treeSelectedNode.label);
	if (cartografia!=null) {
		slider.attr("value",1-cartografia.carTransp/100);
	} 
...
function treeContextMenuInit() {
	...
	dojo.connect(treeContextMenu, "_openMyself", this, function(e) {
        ...
        treeNodeClicked2(null,treeSelectedNode);
...
function createTransparencyDialog() {
	...
	var slider = new dijit.form.HorizontalSlider({
		...
		maximum: 1,
		discreteValues: 1,
		intermediateChanges: true,// executar onChange només al fer mouse up?
...
/**
 * Visibilidad de un nodo del arbol activada o desactivada.
 * @param storeItem
 * @param nodeWidget
 * @return
 */
function treeNodeChecked2(itemId){
	console.log("> treeNodeChecked2 itemId:"+itemId);
	store.fetch( { query: { arnCodigo: itemId },  queryOptions: {deep: true},
		onItem: function(item) {
			console.log("> treeNodeChecked2. onItem:",item);
			item.arnActivo[0]=!item.arnActivo[0];
			
			// mod. los layers OL.
			treeNodeChecked_Recursive(item,item.arnActivo[0]);		
			if (item.arnNombre[0]=="TEXT_MA_POI_BIODIVERSITAT") treeTransparencia(item.cartoid[0])
	} });
...
function treeNodeChecked(storeItem, nodeWidget){
	/*var id;
	if(!storeItem.root) id=storeItem.id;				
	else id=storeItem.id;
	*/
	console.log("> treeNodeChecked. cartoid:"+nodeWidget.item.cartoid);
	
	// mod. los layers OL.
	treeNodeChecked_Recursive(nodeWidget.item,storeItem.arnActivo[0]);
}
 ...
 * @return
 */
function treeNodeChecked_Recursive(item,visibility) {
	...
	if (item.children) {
		for (var i=0; i<item.children.length; i++) {
			treeNodeChecked_Recursive(item.children[i],visibility);
		}
	}
}
...
function mapChangeVisibility(id,cartoid,visibility) {
	...
	try {
		...
		if (visibility){
			...
			if (layers.length>0) {
				console.warn("Error en mapChangeVisibility(). Intentando hacer visible un layer existente.");
...
function mapInit() {
	try {
        ...
        toolsPanel = new OpenLayers.Control.Panel({	
						...
						displayClass: "olControlNGNavigationToolbar"
        ...
        var projeccio= map.projection.replace(":", "_");        
        ...
        mp.formatOutput= function(lonLat) {
			...
			getNls(projeccio) + 
			this.prefix + lonLat.lon.toFixed(0) + this.separator + lonLat.lat.toFixed(0) + this.suffix +
			"</font>";
			return newHtml;
		};
        ...
        // Definimos un seguno panel para poner controles adicionades, normalmente utilizados en tareas.
        /*tasksPanel = new OpenLayers.Control.Panel({	
			div: document.getElementById("tasksTools")}
        );
        map.addControl(tasksPanel);*/
		...
		map.events.register('mousedown', map, function (e) {
			// ha de funcionar conjuntament amb Navigation:handleRightClicks.
	...
	//	map.events.register("zoomend", map, treeRefreshStyleByScale);
		for (var i=0; i<config.olConfig.cartografias.length-1;i++)
		{
			for (var j=0; j<map.layers.length-1; j++)
			{
				if (config.olConfig.cartografias[i].carCodigo==map.layers[j].carCodigo)	
				{
					map.layers[j].setVisibility(config.olConfig.cartografias[i].carVisible);
					//alert(map.layers[j].visibility)
					break;
				}
			}
		}
...
function createLayerOL(cartografia,isFondo,addToMap) {
	try {
		...
		//var serviceUrlParams=getParamsObj(service.stmParamsers,"WMS");
		var serviceUrlParams=getParamsObj(service.stmParamsers,cartografia.stmServicio.serTipo);
		...
		if (window["createLayer"+service.serTipo.toUpperCase()]) {
			...
			if (func==null || typeof(func)!="function") 
				...
				var layer = func(cartografia.carCodigo,url,cartografia.carCapas,serviceUrlParams,otherParams, cartografia, addToMap);
				if (layer != null){
...
function raiseLayerOrder(layer) {
	...
	// recorrem els layers mirant el seu order per calcular la posició on ha d'anar.
		...
		if (typeof(tareas)!="undefined") {
			for (var i=0; i<tareas.length; i++) {
				try {
					...
					//console.log("tareasInit inicializando tarea: "+nombreTarea);
					//console.log(">>>"+typeof(eval("tarea_"+nombreTarea)));
					tareas[i].ref=eval("tarea_"+nombreTarea+"=new tarea_"+nombreTarea+"();");
...
function testSld() {
	var layerOrig=map.getLayersByName("19")[0];
	
	if (layerOrig) {
		var layer=layerOrig.clone();
		layer.setName(layerOrig.name+"-SLD");
		
		var pathSld="http://sitmun2.nexusgeografics.com/sitmun_cime_visor";
		var field="idruta";
		var value="SCE003";
		layer.params.SLD=pathSld+"/sld.jsp?params="+layer.params.LAYERS+"~"+field+"~"+value;
		...
		// utilitzem aquest mètode pq a la versió actual 2.10 encara no hi ha l'event de removed a nivell de layer.
		layerOrig.removeMap=function () {
			layer.destroy();
		};	
	}
}
...
/**
 * Funcion utilizada para llamar a la url para resaltar una ruta
 * @param layer Layer al cual pertenece la ruta
 * @param campo Campo escogido para resaltar la ruta
 * @param valor Valor escogido para resaltar la ruta
 */
function marcarRuta(cartoId, campo, valor){
	console.log("Inicio del marcar ruta");
	console.log("Layer",cartoId);
	...
	layer.redraw();
	map.addLayer(layer);
	//AFEGIM EN L'ORDRE MIG MES PERQUE NO ES QUEDI PER SOTA
	layer.order = layer.order + 0.5;
	raiseLayerOrder(layer);
	// utilitzem aquest mètode pq a la versió actual 2.10 encara no hi ha l'event de removed a nivell de layer.
	/*if (layerOrig) {
		layerOrig.removeMap=function () {
...
function centrarRuta(xmin, ymin, xmax, ymax){
	try{
		console.log("Inici del centrarRuta");
	...
	}catch(e) {console.warn("error en el centrarRuta", e);}
}

function imprimirRuta(){
		//So no el centrem no veurem bé la resolucio
	var xmin=tarea_identify_tur.infoControl.item.XMIN;
	var ymin=tarea_identify_tur.infoControl.item.YMIN;
	var xmax=tarea_identify_tur.infoControl.item.XMAX;
	var ymax=tarea_identify_tur.infoControl.item.YMAX;
	...
	var informacion=dojo.toJson(tarea_identify_tur.infoControl.item);
	var submitForm = getNewSubmitForm();
	submitForm.target="_blank";
	createNewFormElement(submitForm, "informeNombre",  sitmunProperties["identify.informeImpresionRuta"]);
	createNewFormElement(submitForm, "informeDatos", informacion);
	createNewFormElement(submitForm, "mapa1",sitmunProperties["identify.cartografia1ImpresionRuta"]);
	createNewFormElement(submitForm, "mapa2",sitmunProperties["identify.cartografia2ImpresionRuta"]);
	createNewFormElement(submitForm, "lang",getNls("LANG_C"));
	createNewFormElement(submitForm, "logo",sitmunProperties["identify.logoImpresionRuta"]);
	createNewFormElement(submitForm, "sldUrl",sitmunProperties["rutas.sld"]);
	
	var mapMaxExtentString=map.maxExtent.left+":"+map.maxExtent.bottom+":"+map.maxExtent.right+":"+map.maxExtent.top;
	createNewFormElement(submitForm, "tilesMaxExtent", mapMaxExtentString);
	createNewFormElement(submitForm, "tilesResolutions", map.resolutions.join(":"));
	
	var escalas= new Array();
	for(var i=0; i <map.resolutions.length; i++){
		escalas.push(OpenLayers.Util.getScaleFromResolution(map.resolutions[i],'m'));
	}
	createNewFormElement(submitForm, "escalas", escalas.join(":"));
	
	createNewFormElement(submitForm, "tilesWidth",map.tileSize.w);
	createNewFormElement(submitForm, "srs",map.getProjection());

	submitForm.action= "printerRoute";
	submitForm.submit();
	document.body.removeChild(submitForm);
}

function imprimirRutaFlors(){
		//So no el centrem no veurem bé la resolucio
	var xmin=tarea_identify_tur.infoControl.item.ETRSXMIN;
	var ymin=tarea_identify_tur.infoControl.item.ETRSYMIN;
	var xmax=tarea_identify_tur.infoControl.item.ETRSXMAX;
	var ymax=tarea_identify_tur.infoControl.item.ETRSYMAX;
	
	console.log("Inici del imprimirRutaFlors");
	console.log("minx", xmin);
	console.log("miny", ymin);
	console.log("xmax", xmax);
	console.log("ymax", ymax);
	...
	submitForm.target="_blank";
	//createNewFormElement(submitForm, "informeNombre",  sitmunProperties["identify.informeImpresionRuta"]);
	createNewFormElement(submitForm, "informeNombre",  rutaFlors);
	createNewFormElement(submitForm, "informeDatos", informacion);
	createNewFormElement(submitForm, "mapa1",sitmunProperties["identify.cartografia1ImpresionRuta"]);
	createNewFormElement(submitForm, "mapa2",sitmunProperties["identify.cartografia2ImpresionRuta"]);
	createNewFormElement(submitForm, "lang",getNls("LANG_C"));
...
function treeTransparencia(cartoId){
	for (var i=0; i<map.layers.length; i++)
	{
		if (map.layers[i].carCodigo==cartoId)
		{
